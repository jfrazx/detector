...
class FingerPrintController implements CRUD {
  ...
  async update(request: Request, response: Response, next: NextFunction): Promise<void> {
    response.json(await FingerPrint.findByIdAndUpdate(request.params.fingerprint_id, { $set: request.body }));
  }
  async destroy(request: Request, response: Response, next: NextFunction): Promise<void> {
    response.json(await FingerPrint.findByIdAndRemove(request.params.fingerprint_id));
  }
}
